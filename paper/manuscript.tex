...
\begin{highlights}
\item Formalizes enterprise Role-Based Access Control (RBAC) into dynamic Context-Free Grammars.
\item Introduces Ontology-Constrained Token Decoding (O-CTD) to eliminate unauthorized tokens at inference time.
\item Demonstrates an 80\% to 20\% ASR reduction and 90\% to 10\% IVR reduction on real-world enterprise workflows.
\item Achieves 100\% Benign Task Completion (BTC) with verified statistical significance ($p < 10^{-18}, r = 0.800$).
\item Establishes an open-source benchmark and PyTorch/Outlines implementation for reproducible research.
\end{highlights}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/plots/figure1_pareto_tradeoff.pdf}
